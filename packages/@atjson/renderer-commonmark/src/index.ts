...
export default class CommonmarkRenderer extends Renderer {
  ...
  *root() {
   ...
   */
  *Bold(bold: Bold, context: Context) {
  ...
  *Image(image: Image) {
   ...
   */
  *Italic(italic: Italic, context: Context) {
   ...
   */
  *LineBreak(_: any, context: Context) {
  ...
  *Link(link: Link) {
   ...
   * ```
   */
  *Code(code: Code, context: Context) {
    let state = Object.assign({}, this.state);
  ...
  *ListItem() {
   ...
   */
  *List(list: List, context: Context) {
    let start = 1;
