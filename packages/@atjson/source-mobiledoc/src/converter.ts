...
MobiledocSource.defineConverterTo(
  OffsetSource,
  function mobileDocToOffset(doc) {
    doc
    ...
    doc
      .where({ type: "-mobiledoc-hr" })
      .set({ type: "-offset-horizontal-rule" });

    doc.where({ type: "-mobiledoc-ul" }).set({
      type: "-offset-list",
      attributes: { "-offset-type": "bulleted" },
    });
    doc.where({ type: "-mobiledoc-ol" }).set({
      type: "-offset-list",
    ...
    doc
      .where({ type: "-mobiledoc-del" })
      .set({ type: "-offset-strikethrough" });
